...
wget_cookies="$tmp_path/$0.$$.cookies"
#wget_opts="-o $wget_out -O $wget_response -T $timeout -t 1 --progress=dot --save-cookies $wget_cookies --load-cookies $wget_cookies --keep-session-cookies "
wget_opts="-o $wget_out -T $timeout -t 1 --progress=dot --save-cookies $wget_cookies --load-cookies $wget_cookies --keep-session-cookies --save-headers"
...
function do_get {
	...
	url=$(echo $1|sed s/\"//g)
        ...
        wget_response="$tmp_path/$0.$$.GET.$sim_line_number.response"

	rm -f $wget_response
	echo "$base_url$url" > $wget_response
	last_elapsed_secs=$($timer $time_opts wget $wget_opts -O $wget_response "$base_url$url"  2>&1)
	update_elapsed_time
	command_result=$last_elapsed_ms
        find_current_viewstate $wget_response
}
...
function do_post {
	...
	viewstate=$(cat $jsf_viewstate_file)
        #echo do_post har viewstate $viewstate
	#echo do_post har post_data $post_data

        wget_response="$tmp_path/$0.$$.POST.$sim_line_number.response"

	rm -f $wget_response
	last_elapsed_secs=$($timer $time_opts wget $wget_opts -O $wget_response "$base_url$url" --post-data="$post_data&$viewstate" 2>&1)
	echo "$base_url$url" >> $wget_response
	echo "--post-data=$post_data&$viewstate" >> $wget_response
	update_elapsed_time
	command_result=$last_elapsed_ms
        find_current_viewstate $wget_response
}
...
function run_expect {
	...
	if [ "$result" == "0" ] ; then
		...
		command_result="${tp_red}FAILED${tp_rst} $matcher_output"
                failpath="$tmp_path/FAIL-$(basename $wget_response).html"
		echo $args > $failpath
		echo $command_result >> $failpath
		cat $wget_response >> $failpath
	fi
}
...
function execute_every_line_in_script {
	...
	while read line; do
		...
		if emptyline_or_comment $line; then continue; fi
		if is_if_line $line
		then
